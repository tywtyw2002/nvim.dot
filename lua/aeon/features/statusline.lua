...
function c:setup_hl()
    ...
    for name, color in pairs(self.hl) do
        vim.api.nvim_set_hl(
            0,
            "St_copilot_" .. name,
            { fg = hl["St_EmptySpace"].fg, bg = hl[color].fg }
        )
        vim.api.nvim_set_hl(
            0,
            "St_copilot_" .. name .. "Sep",
            { bg = hl["St_EmptySpace"].fg, fg = hl[color].fg }
        )
    end
end
...
M.fileInfo = function()
    ...
    return "%#St_file#" .. icon .. filename .. "%#St_file_sep#" .. sep_r
end
